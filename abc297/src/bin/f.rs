...
fn main() {
    ...
    let mut fac = vec![Mint::new(1)];
    for i in 0..(h * w) {
        fac.push(fac[i] * (i + 1));
    }
    let inv_fac = fac.iter().map(|f| f.inv()).collect_vec();
    ...
    for i in 1..=h {
        for j in 1..=w {
            let area = i * j;
            let rect_num = (h - i + 1) * (w - j + 1);

            let mut comb_num = Mint::new(0);

            for bit in 0..(1 << 4) {
                let hor_remove = (bit & 1) + ((bit >> 1) & 1);
                let ver_remove = ((bit >> 2) & 1) + ((bit >> 3) & 1);
                let corner_remove = hor_remove * ver_remove;
                let sum_remove = hor_remove + ver_remove;

                let minus = j * hor_remove + i * ver_remove - corner_remove;
                ...
                if sum_remove % 2 == 0 {
                    comb_num += calc_combinations(rem, k);
                } else {
                    comb_num -= calc_combinations(rem, k);
                }
            }
