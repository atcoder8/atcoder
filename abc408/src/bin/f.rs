...
fn main() {
    ...
    for (h, &i) in enumerate(&h_to_i) {
        if h >= d {
            let (idx, max) = queue[h - d];
            st.set(idx, S(max));
        }

        let max_num_moves = st.prod(i.saturating_sub(r)..=(i + r).min(n - 1)).0 + 1;
        queue.push((i, max_num_moves));
    }

    let ans = queue.iter().map(|&(_, max)| max).max().unwrap() - 1;
    println!("{}", ans);
}
