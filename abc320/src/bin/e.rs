...
use crate::{binary_search::BinarySearchWithUsize, fenwick_tree::FenwickTree};
...
fn main() {
    ...
    for &(t, w, s) in &tws {
        heap.push((Reverse(t), Reverse(w), s));
    }

    let mut ft: FenwickTree<usize> = FenwickTree::new(n);
    for i in 0..n {
        ft.add(i, 1);
    }
    ...
    while let Some(elem) = heap.pop() {
        if elem.1 .0 != 0 {
            let (Reverse(t), Reverse(w), s) = elem;

            let pos = (0..n).binary_search(|i| ft.sum(0..=i) >= 1, false);
            if let Some(pos) = pos {
                received[pos] += w;
                ft.sub(pos, 1);
                heap.push((Reverse(t + s), Reverse(0), pos));
            }
        } else {
            ft.add(elem.2, 1);
        }
