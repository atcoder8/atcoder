...
fn solve() -> Mint {
    ...
    if n == 2 {
        if s[0] == '?' && s[1] == '?' {
            return Mint::new(ALPHABET_NUM * (ALPHABET_NUM - 1));
        }

        if s[0] == '?' || s[1] == '?' {
            return Mint::new(ALPHABET_NUM - 1);
        }
    ...
    let mut dp = vec![Mint::new(0); ALPHABET_NUM.pow(2)];

    for two_chars in 0..ALPHABET_NUM.pow(2) {
        let cur = two_chars % 26;
        let next = two_chars / 26;

        if !check_conflict((cur as u8 + b'a') as char, s[0])
            && !check_conflict((next as u8 + b'a') as char, s[1])
        {
            dp[cur + next * ALPHABET_NUM] = Mint::new(1);
        }
    }

    for cur_idx in 0..(n - 2) {
        let mut next_dp = vec![Mint::new(0); ALPHABET_NUM.pow(2)];

        for three_chars in 0..ALPHABET_NUM.pow(3) {
            let cur = three_chars % ALPHABET_NUM;
            let next = three_chars / ALPHABET_NUM % ALPHABET_NUM;
            let double_next = three_chars / ALPHABET_NUM.pow(2);

            if !any_conflict(&s, cur_idx, three_chars)
                && (cur_idx >= n - 1 || cur != next)
                && (cur_idx >= n - 2 || next != double_next)
                && (cur_idx >= n - 2 || double_next != cur)
            {
                next_dp[next + double_next * ALPHABET_NUM] += dp[cur + next * ALPHABET_NUM];
            }
        }
    ...
    for two_chars in 0..ALPHABET_NUM.pow(2) {
        ...
        if !check_conflict((cur as u8 + b'a') as char, s[n - 2])
            && !check_conflict((next as u8 + b'a') as char, s[n - 1])
        {
            ans += dp[cur + next * ALPHABET_NUM];
        }
    }
...
fn usize_to_char(x: usize) -> char {
    (x as u8 + b'a') as char
}

fn check_conflict(c1: char, c2: char) -> bool {
    if c1 == '?' || c2 == '?' {
        return false;
    }
...
fn any_conflict(s: &Vec<char>, cur_idx: usize, three_chars: usize) -> bool {
    let n = s.len();

    assert!(cur_idx < n);

    let mut t = three_chars;

    for progress in 0..3 {
        if cur_idx + progress < n
            && check_conflict(usize_to_char(t % ALPHABET_NUM), s[cur_idx + progress])
        {
            return true;
        }
