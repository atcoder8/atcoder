...
fn main() {
    ...
    let mut leader_pairs = HashSet::new();
    for &(x, y) in &xy {
        let (leader_x, leader_y) = (uf.leader(x), uf.leader(y));
        leader_pairs.insert((leader_x, leader_y));
        leader_pairs.insert((leader_y, leader_x));
    }

    for &(p, q) in &pq {
        let (leader_p, leader_q) = (uf.leader(p), uf.leader(q));
        println!(
            "{}",
            if !leader_pairs.contains(&(leader_p, leader_q)) {
                "Yes"
            } else {
                "No"
            }
