...
fn main() {
    ...
    let mut counts_a = vec![0_usize; n + k];
    for &a in &aa {
        counts_a[a] += 1;
    }
    let mut counts_b = vec![0_usize; n + k];
    for &b in &bb {
        counts_b[b] += 1;
    }
    ...
    for &(u, v, w) in &uvw {
        ...
        let leader_u = uf.leader(u);
        let leader_v = uf.leader(v);

        uf.merge(u, v);

        let leader = uf.leader(u);

        counts_a[leader] = counts_a[leader_u] + counts_a[leader_v];
        counts_b[leader] = counts_b[leader_u] + counts_b[leader_v];

        let num_pairs = counts_a[leader].min(counts_b[leader]);
        ans += w * num_pairs;
        counts_a[leader] -= num_pairs;
        counts_b[leader] -= num_pairs;
    }
