use atcoder8_library::{factorial::Factorial, modint::ModInt998244353};
...
fn main() {
    ...
    let odd_degree: usize = graph.iter().map(|x| x.len() % 2).sum();

    let mut ans = Mint::new(0);

    for i in (0..(k + 1)).step_by(2) {
        ans += fac.combinations(odd_degree, i) * fac.combinations(n - odd_degree, k - i);
    }
