...
fn main() {
    ...
    {
        let mut segtree = SegmentTree::<S>::new(n);
        for (i, &p) in enumerate(&pp) {
            let a = p as i64 + i as i64;

            dd[i] = dd[i].min(a - segtree.prod(..p).max);
            segtree.set(p, S::new(a));
        }
    }

    {
        let mut segtree = SegmentTree::<S>::new(n);
        for (i, &p) in enumerate(&pp) {
            let b = p as i64 - i as i64;

            dd[i] = dd[i].min(segtree.prod(p + 1..).min - b);
            segtree.set(p, S::new(b));
        }
    }

    {
        let mut segtree = SegmentTree::<S>::new(n);
        for (i, &p) in enumerate(&pp).rev() {
            let b = p as i64 - i as i64;

            dd[i] = dd[i].min(b - segtree.prod(..p).max);
            segtree.set(p, S::new(b));
        }
    }

    {
        ...
        for (i, &p) in enumerate(&pp).rev() {
            ...
            dd[i] = dd[i].min(segtree.prod(p + 1..).min - a);
            segtree.set(p, S::new(a));
        }
    }

    println!("{}", dd.iter().join(" "));
}
