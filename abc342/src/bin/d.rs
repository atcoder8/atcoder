...
fn main() {
    ...
    let sieve = EratosthenesSieve::new(200000);

    let find_odd_factors = |a: usize| {
        sieve
            .prime_factorization(a)
            .into_iter()
            .filter(|&(_p, cnt)| cnt % 2 == 1)
            .map(|x| x.0)
            .collect_vec()
    };
    ...
    for &a in &aa {
        ...
        let odd_factors = find_odd_factors(a);
        *counter.entry(odd_factors).or_default() += 1;
    }
    ...
    let ans = counter
        ...
        .sum::<usize>()
        + zero_cnt * (n - zero_cnt)
        + zero_cnt * zero_cnt.saturating_sub(1) / 2;
    println!("{}", ans);
}
