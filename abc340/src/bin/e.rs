...
fn main() {
    ...
    let mut lst = LazySegtree::<S, F>::from(aa.iter().map(|&a| S(a)).collect_vec());
    for &b in &bb {
        let ball_num = lst.get(b).0;
        lst.set(b, S(0));
        let cycle_num = ball_num / n;
        let rem = ball_num % n;

        lst.apply_all(F(cycle_num));
        let start = (b + 1) % n;
        let goal = start + rem;
        lst.apply_range(start..goal.min(n), F(1));
        lst.apply_range(..goal.saturating_sub(n), F(1));
    }

    let ans = (0..n).map(|i| lst.get(i).0).join(" ");
    println!("{}", ans);
}
