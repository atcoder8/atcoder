...
use crate::atcoder8_library::binary_search::BinarySearchWithUsize;
...
fn main() {
    ...
    // すれ違う場所を決め打ち
    for &cross_pos in &aa {
        let left_dist = cross_pos;
        let right_dist = l - cross_pos;

        let double_meet_pos = l + right_dist - left_dist;

        // let is_ok = |x| {
        //     double_meet_pos - aa[l]
        // };

        // (0..n).binary_search(is_ok, false);
        ...
        let mut best_l = 0;
        let mut best_r = n - 1;
        let mut best_diff = ((double_meet_pos - aa[l]) - (aa[r] - double_meet_pos)).abs();

        while l < r {
            if double_meet_pos - aa[l] <= aa[r] - double_meet_pos {
                r -= 1;
            } else {
                l += 1;
            }

            let diff = ((double_meet_pos - aa[l]) - (aa[r] - double_meet_pos)).abs();
