...
static MAX_AREA: OnceCell<usize> = OnceCell::new();

fn set_max_area(max_area: usize) {
    MAX_AREA.set(max_area).unwrap()
}

fn get_max_area() -> usize {
    *MAX_AREA.get().unwrap()
}
...
fn main() {
    ...
    let mut lengths = Array2::from_elem((h, w), 0_usize);
    for row in 0..h {
        for col in (0..w).rev() {
            lengths[(row, col)] = if ss[row][col] == '.' {
                if col + 1 < w {
                    lengths[(row, col + 1)] + 1
                } else {
                    1
    ...
    let mut cnt = 0_usize;
    for col in 0..w {
        let st = SegmentTree::from(
            (0..h)
                .map(|row| S {
                    len: 1,
                    streak: lengths[(row, col)],
                    cnt: lengths[(row, col)],
                })
                .collect_vec(),
        );
...
impl Monoid for S {
    ...
    fn prod(&self, rhs: &Self) -> Self {
        ...
        let streak = self.streak.min(rhs.streak);
        let cnt = self.cnt + rhs.cnt + streak.min(get_max_area() / len);
        Self { len, streak, cnt }
    }
}
