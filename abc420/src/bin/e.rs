    ...
    for query in queries {
        match query {
            Query::AddEdge(u, v) => {
                ...
                let u = uf.leader(u);
                let v = uf.leader(v);
                let sum = blacks[u] + blacks[v];
                uf.merge(u, v);
                let leader = uf.leader(u);
                blacks[leader] = sum;
            }
            Query::FlipColor(v) => {
                let color = &mut colors[v];
                let leader = uf.leader(v);
                let num_blacks = &mut blacks[leader];
                if *color {
                    *num_blacks -= 1;
                } else {
                    *num_blacks += 1;
            ...
            Query::Judge(v) => {
                let leader = uf.leader(v);
                println!("{}", if blacks[leader] != 0 { "Yes" } else { "No" });
            }
        }
    }
}
