...
fn main() {
    ...
    let mut sum_dp = vec![vec![Mint::zero(); w]; h];
    let mut cnt_dp = vec![vec![Mint::zero(); w]; h];
    cnt_dp[0][0] = Mint::new(1);

    for i in 0..h {
        for j in 0..w {
            let cur_val = dp[i][j];
            let cur_sum = sum_dp[i][j];
            let cur_cnt = cnt_dp[i][j];

            if i < h - 1 {
                dp[i + 1][j] += cur_val;
                sum_dp[i + 1][j] += cur_sum;
                cnt_dp[i + 1][j] += cur_cnt;

                if ss[i][j] == 'Y' && ss[i + 1][j] == 'Y' {
                    dp[i + 1][j] += Mint::new(2) * cur_sum + cur_cnt;
                    sum_dp[i + 1][j] += cur_cnt;
                }
            }

            if j < w - 1 {
                dp[i][j + 1] += cur_val;
                sum_dp[i][j + 1] += cur_sum;
                cnt_dp[i][j + 1] += cur_cnt;

                if ss[i][j] == 'Y' && ss[i][j + 1] == 'Y' {
                    dp[i][j + 1] += Mint::new(2) * cur_sum + cur_cnt;
                    sum_dp[i][j + 1] += cur_cnt;
                }
            }
        }
    }
