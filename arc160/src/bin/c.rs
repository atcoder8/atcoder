...
fn main() {
    ...
    let mut counts = vec![0_usize; max_a + 20];
    for &a in &aa {
        counts[a] += 1;
    }

    let mut unique_aa = aa.clone();
    unique_aa.sort_unstable();
    unique_aa.dedup();
    unique_aa.insert(0, 0);
    unique_aa.push(max_a + 19);

    let mut dp: HashMap<usize, Mint> = HashMap::new();
    dp.insert(0, Mint::new(1));

    let mut memo = HashMap::new();

    for i in 1..unique_aa.len() {
        let mut next_dp: HashMap<usize, Mint> = HashMap::new();
        ...
        for (&cnt, &comb_num) in &dp {
            for add_cnt in 0..=(cnt / unit) {
                ...
                *next_dp.entry(counts[unique_aa[i]] + add_cnt).or_default() +=
                    comb_num * rec(&mut memo, diff, rem);
            }
        }
...
fn rec(memo: &mut HashMap<(usize, usize), Mint>, diff: usize, rem: usize) -> Mint {
    if let Some(&ret) = memo.get(&(diff, rem)) {
        return ret;
    }
    ...
    for add_cnt in 0..=(rem / unit) {
        let next_rem = rem - unit * add_cnt;
        ret += rec(memo, diff - 1, next_rem);
    }

    memo.insert((diff, rem), ret);
