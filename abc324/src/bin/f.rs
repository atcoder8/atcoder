...
use crate::binary_search::BinarySearchWithF64;

fn main() {
    input! {
        (n, m): (usize, usize),
        mut uvbc: [(Usize1, Usize1, f64, f64); m],
    }
    uvbc.sort_unstable_by_key(|x| x.0);

    let is_ok = |x: f64| {
        let mut dp = vec![-1e8; n];
        dp[0] = 0.0;
        for &(u, v, b, c) in &uvbc {
            let cand_potential = dp[u] + b - x * c;
            if cand_potential > dp[v] {
                dp[v] = cand_potential;
            }
        }
    ...
    let ans = (0.0..10010.0).binary_search(is_ok, 1e-10, true).unwrap();
    println!("{}", ans);
}
