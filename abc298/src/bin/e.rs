...
fn main() {
    input! {
        (n, a, b, p, q): (usize, Usize1, Usize1, usize, usize),
    }
    ...
    let inv_p = Mint::new(p).inv();
    let inv_q = Mint::new(q).inv();

    for i in (0..(n - 1)).rev() {
        for j in (0..(n - 1)).rev() {
            for dice1 in 1..=p {
                let dest1 = (i + dice1).min(n - 1);

                for dice2 in 1..=q {
                    let dest2 = (j + dice2).min(n - 1);

                    let add = dp[dest1][dest2] * inv_p * inv_q;
                    dp[i][j] += add;
                }
            }
        }
