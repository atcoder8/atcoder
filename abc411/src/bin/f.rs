...
fn main() {
    ...
    for &x in &xx {
        let (u, v) = uv[x];
        let (u, v) = (uf.leader(u), uf.leader(v));

        if u != v && graph[u].contains(&v) {
            let mut nodes1 = std::mem::take(&mut graph[u]);
            nodes1.remove(&v);
            let mut nodes2 = std::mem::take(&mut graph[u]);
            nodes2.remove(&u);
            num_edges -= 1;

            if nodes1.len() < nodes2.len() {
                std::mem::swap(&mut nodes1, &mut nodes2);
            }

            let mut merged_node = nodes1;

            uf.merge(u, v);
            let leader = uf.leader(u);

            for node in nodes2 {
                let node = uf.leader(node);
                if merged_node.contains(&node) {
                    num_edges -= 1;
                } else {
                    merged_node.insert(node);
                }
            }
            graph[leader] = merged_node;
        }
