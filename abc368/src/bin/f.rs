...
fn main() {
    ...
    let mut mex_vec = vec![0; max_a + 1];
    for i in 1..=max_a {
        let divisors = sieve.create_divisor_list(i);
        let unique_mex = divisors
            .iter()
            .dropping_back(1)
            .map(|&divisor| mex_vec[divisor])
            .sorted_unstable()
            .dedup()
            .collect_vec();
        mex_vec[i] = enumerate(&unique_mex)
            .position(|(pos, &mex)| pos != mex)
            .unwrap_or(unique_mex.len());
    }

    let grundy = aa.iter().fold(0_usize, |acc, &a| acc ^ mex_vec[a]);
