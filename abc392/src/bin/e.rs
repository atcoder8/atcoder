...
fn main() {
    ...
    let mut leaders: BTreeSet<usize> = (0..n).filter(|&i| uf.leader(i) == i).collect();

    let mut operations = vec![];
    while uf.group_num() != 1 {
        let cable = extra_cables.pop().unwrap();
        let (a, b) = ab[cable];

        let this_leader = uf.leader(a);
        let other_leader = *leaders
            .iter()
            .find(|&&leader| leader != this_leader)
            .unwrap();

        uf.merge(b, other_leader);

        leaders.remove(&this_leader);
        leaders.remove(&other_leader);
        leaders.insert(uf.leader(a));

        operations.push((cable, b, other_leader));
    }
