...
fn main() {
    ...
    let mut numbers = (0..n).collect_vec();
    ...
    for _ in 0..q {
        ...
        if query_type == 1 {
            ...
            if !uf.same(u, v) {
                let mut min = numbers[uf.leader(u)].min(numbers[uf.leader(v)]);

                let mut node = u;
                while {
                    node = pp[node - 1];
                    min = min.min(numbers[uf.leader(node)]);
                    uf.merge(u, node);

                    !uf.same(node, v)
                } {}

                numbers[uf.leader(u)] = min;
            }
        } else {
            input! {
                x: Usize1,
