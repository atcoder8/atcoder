...
fn main() {
    ...
    let zero_num = aa.iter().filter(|&&a| a == 0).count();
    ...
    for i in 1..=m {
        let less_num = aa.iter().filter(|&&a| a < i).count();
        let equal_num = aa.iter().filter(|&&a| a == i).count();

        if less_num >= k || less_num + equal_num + zero_num < k {
            continue;
        }

        for add_equal_num in 1..=zero_num {
            let add_less_num = k - less_num - 1;
            let large_num = k - (add_less_num + add_equal_num);

            let equal_p =
                fac.combinations(zero_num, add_equal_num) * Mint::frac(1, m).pow(add_equal_num);
            let less_p = fac.combinations(zero_num - add_equal_num, add_less_num)
                * Mint::frac(i - 1, m).pow(add_less_num);
            let large_p = Mint::frac(m - k, m).pow(large_num);

            ans += i * less_p * equal_p * large_p;
        }
    }
