...
fn main() {
    ...
    let dfs =
        |start: usize, scores: &mut [Option<i64>], inf_flags: &mut [bool], uf: &mut UnionFind| {
            if scores[start].is_some() {
                return;
            }

            let mut stack = vec![(start, 0)];
            while let Some((cur, score)) = stack.pop() {
                if let Some(decided_score) = scores[cur] {
                    if decided_score != score {
                        inf_flags[uf.leader(cur)] = true;
                ...
                scores[cur] = Some(score);

                for &(next, edge_score) in &graph[cur] {
                    stack.push((next, score + edge_score));
                }
            }
        };

    let mut scores = vec![None; n];
    let mut inf_flags = vec![false; n];
    for start in 0..n {
        dfs(start, &mut scores, &mut inf_flags, &mut uf);
    }

    for &(x, y) in &xy {
        if !uf.same(x, y) {
            println!("nan");
        } else if inf_flags[uf.leader(x)] {
            println!("inf");
        } else {
            println!("{}", scores[y].unwrap() - scores[x].unwrap());
        }
    }
}
