...
fn main() {
    ...
    let mut uf_for_balls = UnionFind::new(n);
    let mut box_to_leader = (0..n).map(|i| Some(i)).collect_vec();
    let mut leader_to_box = (0..(n + q)).collect_vec();
    ...
    for _ in 0..q {
        ...
        if query_type == 1 {
            ...
            match (box_to_leader[x], box_to_leader[y]) {
                (None, None) => {}
                (None, Some(leader_y)) => {
                    box_to_leader[x] = Some(leader_y);
                    box_to_leader[y] = None;
                    leader_to_box[leader_y] = x;
                }
                (Some(_), None) => {}
                (Some(leader_x), Some(leader_y)) => {
                    uf_for_balls.merge(leader_x, leader_y);
                    let updated_leader_x = uf_for_balls.leader(leader_x);
                    box_to_leader[x] = Some(updated_leader_x);
                    box_to_leader[y] = None;
                    leader_to_box[updated_leader_x] = x;
                }
            }
            ...
            if let Some(leader_x) = box_to_leader[x] {
                uf_for_balls.merge(leader_x, uf_for_balls.elem_num() - 1);
                let updated_leader_x = uf_for_balls.leader(leader_x);
                box_to_leader[x] = Some(updated_leader_x);
                leader_to_box[updated_leader_x] = x;
            } else {
                box_to_leader[x] = Some(uf_for_balls.elem_num() - 1);
                leader_to_box[uf_for_balls.elem_num() - 1] = x;
            }
        } else {
            input! {
                x: Usize1,
            ...
            let ans = leader_to_box[uf_for_balls.leader(x)] + 1;
            println!("{}", ans);
        }
    }
}
