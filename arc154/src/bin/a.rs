...
fn main() {
    ...
    let cc = aa.iter().zip(&bb).map(|(a, b)| *a.min(b)).collect_vec();
    let dd = aa.iter().zip(&bb).map(|(a, b)| *a.max(b)).collect_vec();

    let c_mod = cc.iter().fold(Mint::zero(), |acc, &c| 10 * acc + c);
    let d_mod = dd.iter().fold(Mint::zero(), |acc, &d| 10 * acc + d);
