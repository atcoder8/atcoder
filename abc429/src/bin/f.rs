...
fn main() {
    ...
    let mut st = SegmentTree::from(
        (0..n)
            .map(|col| Some(S::new(std::array::from_fn(|row| ss[row][col]))))
            .collect_vec(),
    );
    ...
    let output = rc
        .iter()
        .map(|&(row, col)| {
            ss[row][col] = if ss[row][col] == '.' { '#' } else { '.' };
            st.set(col, Some(S::new(std::array::from_fn(|row| ss[row][col]))));
            match st.all_prod().unwrap().dist(0, 2) {
                Some(dist) => Cow::Owned(dist.to_string()),
                None => Cow::Borrowed("-1"),
            }
        })
        .join("\n");
...
struct S([Option<usize>; 9]);

impl S {
    fn new(column_chars: [char; 3]) -> Self {
        let calc_dist = |from: usize, to: usize| {
            let col1 = from.min(to);
            let col2 = from.max(to);

            if column_chars[col1..=col2].iter().all(|&ch| ch == '.') {
                Some(col2 - col1)
            } else {
                None
            }
        ...
        Self(std::array::from_fn(|i| calc_dist(i / 3, i % 3)))
    }

    fn dist(&self, from: usize, to: usize) -> Option<usize> {
        self.0[3 * from + to]
    }
}

impl Monoid for Option<S> {
    fn id() -> Self {
        None
    }

    fn prod(&self, rhs: &Self) -> Self {
        let (Some(lhs), Some(rhs)) = (*self, *rhs) else {
            return self.or(*rhs);
        };

        let calc_composited_dist = |from: usize, to: usize| {
            (0..3)
                .filter_map(|mid| match (lhs.dist(from, mid), rhs.dist(mid, to)) {
                    (Some(dist1), Some(dist2)) => Some(dist1 + dist2 + 1),
                    _ => None,
                })
                .min()
        };
        Some(S(std::array::from_fn(|i| {
            calc_composited_dist(i / 3, i % 3)
        })))
    }
}
