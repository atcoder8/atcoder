...
const MAX_A: usize = 10_usize.pow(12);
...
fn main() {
    ...
    for (i, &p) in enumerate(&primes) {
        ...
        let mut raised_p = 1_usize;
        while raised_p <= MAX_A / p.pow(2) {
            raised_p *= p.pow(2);

            for &q in &primes[i + 1..] {
                let mut raised_q = 1_usize;
                while raised_p * raised_q <= MAX_A / q.pow(2) {
                    raised_q *= q.pow(2);
                    numbers.push(raised_p * raised_q);
                }
            }
        }
    }
    ...
    let solve = |a: usize| {
        let pos = numbers.upper_bound(&a);
        numbers[pos - 1]
    };

    println!("{}", aa.iter().cloned().map(solve).join("\n"));
}
