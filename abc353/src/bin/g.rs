...
fn main() {
    ...
    st.set(
        0,
        S {
            to_right: -c * (n - 1) as i64,
            to_left: 0,
        },
    );

    for &(t, p) in &tp {
        let max = (st.prod(..=t).to_right + c * (n - 1 - t) as i64)
            .max(st.prod(t..).to_left + c * t as i64)
            + p;

        st.set(
            t,
            S {
                to_right: max - c * (n - 1 - t) as i64,
                to_left: max - c * t as i64,
            },
        );
    }
    ...
    let mut ans = 0;
    for i in 0..n {
        let S { to_right, to_left } = st.get(i);
        ans = ans
            .max(to_right + c * (n - 1 - i) as i64)
            .max(to_left + c * i as i64);
    }
...
impl Monoid for S {
    ...
    fn prod(&self, rhs: &Self) -> Self {
        S {
            to_right: self.to_right.max(rhs.to_right),
            to_left: self.to_left.max(rhs.to_left),
        }
    }
}
