...
fn main() {
    ...
    let mut memo: HashMap<usize, Mint> = HashMap::new();
    memo.insert(n, Mint::new(1));
    let ans = rec(&mut memo, 1, n);
    println!("{}", ans.val());
}

fn rec(memo: &mut HashMap<usize, Mint>, x: usize, n: usize) -> Mint {
    if let Some(&ret) = memo.get(&x) {
        return ret;
    }
