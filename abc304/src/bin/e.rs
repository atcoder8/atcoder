...
fn main() {
    ...
    let leader_pairs: HashSet<(usize, usize)> = xy
        .iter()
        .map(|&(x, y)| (uf.leader(x), uf.leader(y)))
        .collect();

    for &(p, q) in &pq {
        let (p, q) = (uf.leader(p), uf.leader(q));
        let ans = !leader_pairs.contains(&(p, q)) && !leader_pairs.contains(&(q, p));
